...
void InitializeTasks(int* tasks, int iteration) {
    for (int i = 0; i < NUM_OF_TASKS; i++) {
        tasks[i] = abs(50 - i % 100) * abs(rank - (iteration % size)) * L;
    }
}
...
void* Work(void* ignored) {
    ...
    for (int i = 0; i < NUM_OF_ITERATIONS; i++) {
        ...
        while (true) {
            if (enough_tasks == true) {
                break;
            }
            int responsed_total = 0;
            int requested = 0;
            for (int i = 0; i < size; i++) {
                if (i == rank) {
                    continue;
                }
                MPI_Send(&rank, 1, MPI_INT, i, ASK_FOR_TASKS_TAG , MPI_COMM_WORLD);
                MPI_Recv(&responsed, 1, MPI_INT, i, SEND_NUM_OF_TASKS_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                requested++;
                responsed_total += responsed;
                if (responsed_total == 0 && requested == size - 1) {
                    enough_tasks = true;
                    break;
                }
                if (responsed == 0) {
                ...
                MPI_Recv(task_list, supplied_tasks, MPI_INT, i, SEND_TASKS_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE) ;
                pthread_mutex_lock(&lock);
