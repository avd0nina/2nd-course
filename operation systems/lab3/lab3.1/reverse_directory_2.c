...
void ReverseFileContent(const char *src_path, const char *dst_path) {
    ...
    off_t remaining = size_file;
    char buffer[4096];
    while (remaining > 0) {
        off_t chunk_size = (remaining >= sizeof(buffer)) ? sizeof(buffer) : remaining;
        remaining -= chunk_size;
        lseek(src_fd, remaining, SEEK_SET);
        read(src_fd, buffer, chunk_size);
        write(dst_fd, buffer, chunk_size);
    }
    close(src_fd);
    close(dst_fd);
}
